...
# Database configuration using the default MySQL user
db_config = {
    'host': 'localhost',
    'user': 'root',       # default MySQL user
    'password': '',       # default: no password (update if necessary)
    'database': 'airline_db'
}
...
# Function to fetch all mobile numbers from the MySQL database
def fetch_mobile_numbers():
    try:
        ...
        # Query to fetch all mobile numbers from the 'demo' table
        query = "SELECT mobile FROM demo"
...
def initiate_call(to_number):
    try:
        ...
        # Make the call using a TwiML URL that provides instructions for the call
...
def main():
    ...
    if mobile_numbers:
        # Initiate calls to all fetched numbers
